...
class ETFWeightedSlopeRotationStrategy(BaseStrategy):
    ...
    def __init__(self, strategy_cfg: dict) -> None:
        ...
        self._frozen_symbols: dict[str, int] = {}
    ...
    def _advance_freeze_days(self) -> None:
        self._frozen_symbols = {
            symbol: days - 1
            for symbol, days in self._frozen_symbols.items()
            if days - 1 > 0
        }
        ...
        if self._active_symbol:
            ...
            if not active_row.empty:
                ...
                if self._active_high > 0 and close <= self._active_high * (1.0 - self.stop_profit_drawdown):
                    self._frozen_symbols[self._active_symbol] = self.cooldown_days
                    self._reset_state()

        df["score"] = df[score_col]
        df = df[(df["score"] >= self.min_score) & (df["score"] <= self.max_score)]
        if self._frozen_symbols:
            df = df[~df["symbol"].isin(self._frozen_symbols)]
        if df.empty:
            self._reset_state()
            return df
