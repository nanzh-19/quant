    ...
    for index, (trade_date, day_df) in enumerate(daily_groups):
        ...
        if rebalance:
            ...
            if ranked.empty:
                ...
                if current_weights:
                    buy_fraction = 0.0
                    sell_fraction = sum(current_weights.values())
                ...
                if "weight" in ranked.columns:
                    ranked["weight"] = pd.to_numeric(ranked["weight"], errors="coerce").fillna(0.0)
                    ranked = ranked[ranked["weight"] > 0]
                    total_weight = float(ranked["weight"].sum())
                    target_weights = {
                        str(row.symbol): float(row.weight) / total_weight
                        for row in ranked.itertuples(index=False)
                    } if total_weight > 0 else {}
                else:
                    new_symbols = set(ranked["symbol"])
                    equal_weight = 1.0 / len(new_symbols) if new_symbols else 0.0
                    target_weights = {symbol: equal_weight for symbol in new_symbols}
                all_symbols = set(current_weights) | set(target_weights)
                buy_fraction = sum(max(target_weights.get(symbol, 0.0) - current_weights.get(symbol, 0.0), 0.0) for symbol in all_symbols)
                sell_fraction = sum(max(current_weights.get(symbol, 0.0) - target_weights.get(symbol, 0.0), 0.0) for symbol in all_symbols)
                ...
                selection_cols = [col for col in ["date", "symbol", "name", "score", "weight", "close", "reason"] if col in ranked.columns]
        ...
        returns_by_symbol = {
            ...
            for row in day_df[day_df["symbol"].isin(current_weights)].itertuples(index=False)
            if pd.notna(row.fwd_ret_1)
        }

        gross_return = sum(weight * returns_by_symbol.get(symbol, 0.0) for symbol, weight in current_weights.items())
        net_return = gross_return - cost

        if gross_return > -1.0:
            current_weights = {
                symbol: weight * (1.0 + returns_by_symbol.get(symbol, 0.0)) / (1.0 + gross_return)
